...
from api.priceapp.models import PriceSubmission
from api.priceapp.serializers import PriceSubmissionSerializer
...
def create(self, validated_data):
    ...
    validated_data['verified'] = user.is_authenticated
...
class PriceSubmissionView(APIView):
    def post(self, request):
        ...
        if request.content_type == "application/json":
            ...
            receipt_file = request.FILES.get("receipt", None)
            receipt_path = None
            if receipt_file:
                receipt_path = default_storage.save(f'receipts/{receipt_file.name}', ContentFile(receipt_file.read()))
        ...
        submission = PriceSubmission.objects.create(
            ...
            receipt=receipt_path if file else None,
        )
